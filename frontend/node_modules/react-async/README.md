...
- `promise` {Promise} the last evaluated promise
...
### Nested Async components with dependency using Promise.all

```js
<Async promiseFn={loadSummary}>
  {({ data: summary, promise }) => (
    <Async promiseFn={() => Promise.all([loadDetails, promise])}>
